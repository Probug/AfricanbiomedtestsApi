...
namespace Africanbiomedtests.Services
{
    public interface IHealthcareProviderService
    {
        ...
        IEnumerable<HealthcareProviderAccountResponse> GetAll();
        HealthcareProviderAccountResponse GetById(int id);
        HealthcareProviderAccountResponse Create(HealthcareProviderCreateRequest model);
        HealthcareProviderAccountResponse Update(int id, HealthcareProviderUpdateRequest model);
        void Delete(int id);
    }
    ...
    public class HealthcareProviderService : IHealthcareProviderService
    {
        ...
        {
            var account = _context.HealthcareProviders.SingleOrDefault(x => x.Email == model.Email);
        ...
        {
            var account = _context.HealthcareProviders.SingleOrDefault(x => x.VerificationToken == token);
        ...
        public IEnumerable<HealthcareProviderAccountResponse> GetAll()
        {
            var accounts = _context.HealthcareProviders;
            return _mapper.Map<IList<HealthcareProviderAccountResponse>>(accounts);
        }

        public HealthcareProviderAccountResponse GetById(int id)
        {
        ...
        {
            var account = getAccount(id);
            _context.HealthcareProviders.Remove(account);
            _context.SaveChanges();
        }

        // helper methods

        private HealthcareProvider getAccount(int id)
        {
            var account = _context.HealthcareProviders.Find(id);
            if (account == null) throw new KeyNotFoundException("Account not found");
            return account;
        }

        private (RefreshToken, HealthcareProvider) getRefreshToken(string token)
        {
            var account = _context.HealthcareProviders.SingleOrDefault(u => u.RefreshTokens.Any(t => t.Token == token));
